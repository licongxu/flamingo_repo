"""Build M500c>5e13 L2p8_m9 lightcone catalogues with q_from_mz (Route B, GPU, B=1.35).

Streams the existing M>=1e13 catalogues for lightcones 0..7, keeps only
M_500c > 5e13 Msun, computes parametric A10 GNFW SNR with lognormal intrinsic
scatter via hmfast/JAX on GPU, and writes compact derived catalogues to RDS:

    lightcone{N}/catalogues/halo_catalogue_M500c_5e13_zlt3_L2p8_m9_yang26rot_qfrommz.csv

Run:
    python scripts/add_q_from_mz_L2p8_m9_catalogues.py
    python scripts/add_q_from_mz_L2p8_m9_catalogues.py --lightcone 3
"""
...
def src_path(lightcone: int) -> Path:
    return (
        BASE
...
def out_path(lightcone: int) -> Path:
    return (
        BASE
        / f"lightcone{lightcone}"
        / "catalogues"
...
def process_catalogue(
    ...
    *,
    ...
    src = src_path(lightcone)
    dst = out_path(lightcone)
    if not src.exists():
        print(f"skip {label}: missing {src}")
        return 0, 0, 0
    if dst.exists() and not force:
        print(f"skip {label}: exists {dst.name}")
        n_elig = n_q5 = 0
    ...
    with tmp.open("w") as out:
        out.write(
            f"# L2p8_m9 lightcone{lightcone}: M_500c > {M_MIN:.0e} Msun subset with q_from_mz.\n"
            f"# Source: {src.name}\n"
        ...
        for ichunk, chunk in enumerate(pd.read_csv(src, comment="#", chunksize=CHUNK)):
            ...
            if (ichunk + 1) % 5 == 0:
                print(
                    f"  {label}: scanned {n_scanned:,}, kept {n_kept:,} "
                    f"({time.time()-t0:.0f}s)",
                    flush=True,
                )
    ...
    tmp.replace(dst)
    print(
        f"{label}: kept {n_kept:,}/{n_scanned:,}, N(q>5)={n_q5:,} "
...
def main() -> None:
    ...
    for lc in lightcones:
        print(f"=== lightcone{lc} ===", flush=True)
        summary.append((f"lc{lc}", *process_catalogue(lc, hm, a_sz, coeff, force=args.force)))

    print(f"\n=== summary: M_500c > {M_MIN:.0e}, N(q>5) ===")
    for label, _nscan, n_kept, n_q5 in summary:
        print(f"  {label:28s}  kept={n_kept:8,}  N(q>5)={n_q5:6,}")
