...
Run::
    ...
    python scripts/powerspectra/masking_radius_random_control.py --variant L1_m9 --q-cut 5
"""
...
from scripts.powerspectra import masking_radius_null_test as nt  # noqa: E402
...
def main() -> None:
    ...
    for i, r_mult in enumerate(nt.R_MULTS):
        ...
        cl = nt.decoupled_cl_per_ell(ymap, mask_apo, pixwin2, lmax=nt.LMAX)
        del mask_apo
        ell_log, dl_12 = nt.bin_log_dl(ell, cl)
        np.savez(
            out,
            variant=variant,
            q_cut=cut,
            ...
            f_sky_raw=f_sky_raw,
            f_sky_eff=f_sky_eff,
            ell_18=nt.PLANCK_ELL_EFF,
            dl_18=nt.bin_planck_dl(ell, cl) * 1e12,
            ell_log=ell_log,
            dl_12=dl_12 * 1e12,
            nside=nside,
        )
