#!/usr/bin/env python3
"""Summarize and plot the L1_m9 A_SZ chain with alpha_SZ fixed."""
...
def main() -> None:
    ...
    plotter.plot_1d(samples, "A_SZ", colors=["#2c7fb8"])
    plotter.add_x_marker(best_asz, color="#d7301f", lw=1.4)
    for suffix in ("png", "pdf"):
        plotter.export(str(CHAIN_DIR / f"posterior_A_SZ.{suffix}"))
    plt.close("all")
    print("\n".join(lines))
