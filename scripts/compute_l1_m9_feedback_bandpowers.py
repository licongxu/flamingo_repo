...
Run::
    ...
    python scripts/compute_l1_m9_feedback_bandpowers.py --workers 4
"""
...
def load_catalogue(path: Path, q_column: str = "q_from_mz") -> dict[str, np.ndarray]:
    """Stream the catalogue, retaining only masking and rotation-check columns."""
    ...
    requested = [q_column if column == "q_from_mz" else column for column in COLUMNS]
    for chunk in pd.read_csv(
        path,
        comment="#",
        usecols=requested + SAMPLE_COLUMNS,
        chunksize=CHUNK,
    ):
    ...
    out = {key: np.concatenate(value) for key, value in cols.items() if key != "nat"}
