"""Build SNR catalogue using SOAP Y_500c (R500c aperture) and GNFW K(r_out=1).

Same B=1.35 hydrostatic-bias conventions as ``build_y0q_catalogue_arnaudB135.py``,
but uses the SOAP Compton-Y integrated within R_500c (not 5 R_500c) and the
matching GNFW geometric factor K(r_out=1).

Run:
    JAX_PLATFORMS=cpu python scripts/build_y0q_catalogue_arnaudB135_Y500c.py
"""
...
OUT = _REPO / f"data/hydro_L2p8m9/catalogue/halo_catalogue_M500c_5e13_zlt3_y0q_arnaudB{B_TAG}_Y500c.csv"
...
def main() -> None:
    ...
    amp_factor = (1.0 / B) ** GAMMA_Y0
    print(f"GNFW K(r_out={R_OUT}) = {K:.6f}  (y0 = K * {Y_COL} / R_500c**2)")
    print(f"B = {B}:  amplitude (1/B)^{GAMMA_Y0} = {amp_factor:.6f};  theta (1/B)^(1/3) = {b_geom:.6f}")
    ...
    header = (
        f"# FLAMINGO L2p8_m9: derived y0 and SNR (A10 GNFW, B={B}, Y_R500c aperture).\n"
        f"# SOAP {Y_COL} with GNFW K(r_out={R_OUT}) = {K:.6f}:\n"
        f"# y0 = {K:.6f} * {Y_COL} / R_500c**2 * (1/B)^{GAMMA_Y0}\n"
        "# theta500_arcmin = R_500c*(1/B)^(1/3) / D_A(z); sigma_y0 from szifi immf6.\n"
        "# q = y0 / sigma_y0.\n"
    )
