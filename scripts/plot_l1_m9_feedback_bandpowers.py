"""Plot the L1_m9 feedback-variant tSZ bandpowers (full sky and q>5 masked).

Two figures, one per binning convention in ``data_paper/feedback_bandpower``:

* 18 inclusive Planck bins (``..._binned_18.txt``)
* 12 logarithmic bins, ``Delta ln ell = 0.4``, ``ell_max = 10000``

Each figure has two columns (full sky, q>5 masked) and two rows (bandpowers,
ratio to the fiducial run).
...
def _figure(
    *,
    ...
    for col, masked in enumerate((False, True)):
        ...
        ax.set_title("full sky" if not masked else r"masked, $q>5$", fontsize=10)
        ax.set_xscale("log")
        axr.set_xscale("log")
