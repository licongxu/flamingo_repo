"""NaMaster tSZ bandpowers for B=1.35 Y_R500c on the unrotated z<0.35 map.

Uses:
  - y_unlensed_L2p8_m9_lc0_unrotated_z0_0p35_nside4096.fits
  - arnaudB135 Y500c q catalogue (clusters with z < 0.35 only for masking)
  - yang26-rotated SOAP positions (identity rotation for z < 0.35)

Modes:
  qgt5    — mask 5×θ500 discs around q>5 clusters (z < 0.35)
  fullsky — no cluster masking

Outputs (under data/bandpowers_arnaudB135_Y500c_unrot_z0p35/):
  Dl_yy_unrot_z0p35_qgt5_binned_18_Y500c.txt
  Dl_yy_unrot_z0p35_fullsky_binned_18_Y500c.txt
...
Y_MAP = _REPO / "data/map/y_unlensed_L2p8_m9_lc0_unrotated_z0_0p35_nside4096.fits"
Y0Q = _REPO / "data/hydro_L2p8m9/catalogue/halo_catalogue_M500c_5e13_zlt3_y0q_arnaudB135_Y500c.csv"
YANG26 = _REPO / "data/hydro_L2p8m9/catalogue/halo_catalogue_M500c_5e13_zlt3_soap_snr_d3a_yang26rot.csv"
OUT_DIR = _REPO / "data/bandpowers_arnaudB135_Y500c_unrot_z0p35"
...
def _build_mask(ymap, nside, log) -> tuple[np.ndarray, int, float]:
    ...
    z = y0q["z"].to_numpy(float)
    q = y0q["q"].to_numpy(float)
    sel = np.isfinite(q) & (q > Q_CUT) & (z < Z_MAX_MAP)
    n_cl = int(sel.sum())
    ...
    log(
        f"  yang26 positions, z<{Z_MAX_MAP}: N(q>{Q_CUT})={n_cl:,}  "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="qgt5: masked q>5 catalogue (z<0.35); fullsky: unmasked map",
    )
    ...
    if args.mode == "qgt5":
        out_txt = OUT_DIR / "Dl_yy_unrot_z0p35_qgt5_binned_18_Y500c.txt"
        out_meta = OUT_DIR / "meta_unrot_z0p35_qgt5_Y500c.npz"
    else:
        out_txt = OUT_DIR / "Dl_yy_unrot_z0p35_fullsky_binned_18_Y500c.txt"
        out_meta = OUT_DIR / "meta_unrot_z0p35_fullsky_Y500c.npz"

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    log(f"mode={args.mode}  B=1.35  z_max={Z_MAX_MAP}  unrotated map")
    ...
    if args.mode == "qgt5":
        ...
        meta_extra = dict(n_masked=n_cl, q_cut=Q_CUT, z_max=Z_MAX_MAP, B=1.35)
    else:
        mask_apod, fsky_eff = _build_fullsky_mask(ymap, log)
        meta_extra = dict(n_masked=0, q_cut=None, z_max=Z_MAX_MAP, B=1.35)
