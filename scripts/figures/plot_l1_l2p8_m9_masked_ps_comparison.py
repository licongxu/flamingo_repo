...
Run::
    ...
    python scripts/figures/plot_l1_l2p8_m9_masked_ps_comparison.py
"""
...
DATA = DATA_ROOT / "data_paper" / "binned_bandpowers"
FIGURES = DATA_ROOT / "figures" / "masked_ps"
...
def _paths(
    ...
    *,
    ...
    return data / (
        f"Dl_yy_{variant}_masked_{tag}_{selection_tag}_{suffix}.txt"
    )


def _fullsky_paths(variant: str, *, log: bool, data: Path = DATA) -> Path:
    if log:
        return data / (
...
def _save(fig: plt.Figure, stem: Path) -> None:
    ...
    for suffix in ("png", "pdf"):
        ...
        print(f"wrote {stem.with_suffix('.' + suffix)}", flush=True)
    plt.close(fig)
...
def _plot(
    *,
    ...
    stem: Path,
    selection_tag: str,
    resolutions: tuple[tuple[str, str, str, str], ...],
    data: Path = DATA,
) -> None:
    ...
    for _, masked_variant, fullsky_variant, ls in resolutions:
        ell_fs, dl_fs = _load(
            _fullsky_paths(fullsky_variant, log=log, data=data)
        )
...
def main() -> None:
    ...
    if args.selection == "qfrommap":
        resolutions = (
            ("L1_m9", "L1_m9", "L1_m9", "-"),
            ("L2p8_m9", "L2p8_m9_lc0", "L2p8_m9", "--"),
        )
        title_q = "empirical aperture q"
        stem_tag = "qfrommap"
    else:
        resolutions = (
            ("L1_m9", "L1_m9", "L1_m9", "-"),
            ("L2p8_m9", "L2p8_m9", "L2p8_m9", "--"),
        )
        title_q = r"$\alpha_{\rm SZ}=1.12$ best-fit $q$"
        stem_tag = "alpha_fixed_1p12"
    ...
    _plot(
        ...
        title=(
            ...
            f"18 Planck bins, {title_q}"
        ),
        stem=FIGURES / f"l1_l2p8_m9_masked_ps_binned_18_{stem_tag}",
    ...
    _plot(
        ...
        title=(
            "FLAMINGO masked tSZ: L1_m9 (solid) vs L2p8_m9 (dashed), "
            rf"$\Delta\ln\ell=0.4$ log bins, {title_q}"
        ),
        stem=FIGURES / f"l1_l2p8_m9_masked_ps_logbins_{stem_tag}",
        selection_tag=args.selection,
        resolutions=resolutions,
    )
