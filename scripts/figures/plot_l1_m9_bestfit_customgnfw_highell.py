...
def compute_theory_bandpowers(
    ...
    *,
    ...
    """Evaluate and log-bin custom-GNFW D_ell terms."""
    chunks = []
    for index, (lower, upper) in enumerate(zip(edges[:-1], edges[1:])):
...
def make_high_ell_figure(
    ...
    upper.loglog(
        ...
        color="#e67e22",
        linewidth=2.2,
        label="customGNFW fixed-alpha fit, total",
    )
    ...
    upper.text(
        ...
        (
            ...
            rf"$B={B:.2f}$"
            "\n"
            rf"fixed $\alpha_{{\rm SZ}}={FIXED_ALPHA_SZ:.2f}$: "
            rf"$A_{{\rm SZ}}={FIXED_ALPHA_A_SZ:.7f}$"
            "\n"
            rf"$\Delta\ln\ell=0.4$"
        ),
...
if __name__ == "__main__":
    ...
    fixed_alpha_model = compute_theory_bandpowers(
        log_edges,
        A_SZ=FIXED_ALPHA_A_SZ,
        alpha_SZ=FIXED_ALPHA_SZ,
    )
    simple_gnfw = {
    ...
    figure = make_high_ell_figure(
        ...
        fixed_alpha_model["total"] * DISPLAY_SCALE,
    )
